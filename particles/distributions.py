...
class Binomial(DiscreteDist):
    ...
    def rvs(self, size=None):
        return random.binomial(self.n, self.p, size=size).astype('int8')
...
class Mixture(ProbDist):
    ...
    *components: ProbDist objects
    ...
    def logpdf(self, x):
        ...
        return scp.logsumexp(np.column_stack(tuple(lpks)),
                                    axis=-1)
...
class IndepProd(ProbDist):
    ...
    def __init__(self, *dists):
        ...
        if all(d.dtype == 'int64' for d in dists):
            self.dtype = 'int64'
        if all(d.dtype == 'int8' for d in dists):
            self.dtype = 'int8'
        else:
            self.dtype = 'float64'
